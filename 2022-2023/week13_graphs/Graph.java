...
public class Graph<T> {
    ...
    public void connectedComponents() {
        // print connected components with the same color, if we have more components than colors it will crash 🙂
...
//        }
    ...
    public ArrayList<T> shortestPath(T a, T u) {
        // run BFS first
        HashMap<T, Integer> distances = new HashMap<>();
        ...
        distances.put(a, 0); // distance from itself is 0
        Queue<T> queue = new ArrayDeque<>();
        queue.add(a);
        while (!queue.isEmpty()) {
            T v = queue.remove();
        ...
        if(distances.get(u) == Integer.MAX_VALUE)
            throw new IllegalStateException("No path from " + a + " to " + u);
        return reconstructPath(a, u, prev);
    }
